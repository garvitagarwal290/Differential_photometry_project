...
def getRAdec(transform, xy):
    ...
    #get transformation
    world = transform.all_pix2world(x,y, 0,ra_dec_order=True) 
...
def initialFindFITS(data, detect_thresh, target_mask):
    ...
    thresh = detect_thresh * bkg.globalrms      # set detection threshold to mean + detect_thresh* sigma
    ...
    ''' Generate tuple of positions for each star'''
...
def get_correct_hdrfile_idx(num_images, filenames, data_path): 
    ...
    coords derived from the user defined ra dec. This gives us the one image in the sequence that is guaranteed to have a correct transform from
    ra dec to pixels'''
...
#splits each stare/observation_session further if the telescope itself changes during the observation (like in LCO observatory)
...
#this function locates the stars in the field. It excludes the sources that are close to the edge or are too elliptical to be
#stars. 
def get_radec_stars():
    ...
    #find an image with correct radec to pixel transformation information in the header
    ...
    #mask the target before finding rest of the stars using SEP
    hdu = fits.open(filenames[correct_hdrfile_idx])
    x,y=getXY(hdu[1].header, np.array([target_radec[k]]))
    target_x=x[0]
    target_y=y[0]
    target_mask = np.zeros_like(correct_hdrframe[0])
    mask_size = [int(x_length * 0.025), int(y_length * 0.025)]
    target_mask[round(target_x-mask_size[0]):round(target_x+mask_size[0]), round(target_y-mask_size[1]):round(target_y+mask_size[1])] = 1.0
    ...
    #if there are too many stars, reduce aperture size to account for crowded fields
    global ap_r
    if(before_ellipticity_filter) > 10000: ap_r = 5
    else: ap_r = 10

    #remove sources that are too elliptical, since they are likely galaxies
    ...
    #remove stars that are too close to edge of frame
...
#this function selects the reference star for the target. it calculates the median and stddev of the residual of target's 
# normalised LC with that of a reference star and selects those stars for which the median and stddev is within the smallest 10%
def choose_referencestars():
    chosenstars_idx = []
    weights = []
    
    for measured_star in range(0,1):#,num_stars): #loop through each star
        median = []
        stdev=[]
        ...
        for star in range(0, num_stars): #loop through all stars
            if star!=measured_star: 
                reference_star_allflux = np.concatenate([dirdata[:,star,2] for dirdata in alldir_data])
                ref_flux= reference_star_allflux/abs(np.mean(reference_star_allflux))  #get their relative flux
        ...
        #to find stars with smallest 10% median and stddev
        ...
        #calculate weights for each comparison star based on the stddev of its normalised flux
        for star in chosenstars_idx:
            reference_star_allflux = np.concatenate([dirdata[:,star,2] for dirdata in alldir_data])
            ref_flux= reference_star_allflux/np.mean(reference_star_allflux) #normalized lightcurve of target star
            try:
                weights.append((1/stats.sigma_clipped_stats(ref_flux)[2])**2)
...
'''Garvit Ag: My modified version of Roman's code:
        ...
        2) To select reference stars for the target, replaces the 'chisquare' score with a new score that finds the median and stdev of the residuals of each star 
...
#Get names of all targets
data_parentpath = '/home/gagarwal/Downloads/Data/'
parent_savefolder = '/home/gagarwal/Downloads/relative_LCs/'
tempfile_path = '/home/gagarwal/Downloads/'
os.system(" ls -d {}*/ > {}temp.txt".format(data_parentpath, tempfile_path)) 
stars = [starpath.split('/')[-2] for starpath in open('/home/gagarwal/Downloads/temp.txt', 'r').read().split('\n')[:-1]]

#The user needs to provide the most accurate ra dec (in deg) of the targets in a list in the same order as they are in the list 'stars'
target_radec = [[216.4454064, -3.987855961], [233.73115833, -14.31575278], [255.071998, -16.91276414], [262.8633877, -7.997966847], [267.212726, -0.649488056], [269.38794583, -12.89951389], [276.0985121, -5.615670083], [283.0641767, -3.097493986], [316.0877851, -69.8785669], [163.6290162, 25.96294055], [296.40258, -25.957214], [38.8217307, 15.7235842], [205.9105716, 8.429159262], [97.183275, 5.484615], [241.8800931, -4.704552696], [258.0163403, -3.392545346]]

detect_thresh=4 #threshold pixel value for detecting stars is taken to be detect_thresh * background_rms
inner_annulus = 17 #photometry bkg inner anulus (in pixels)
outer_annulus = 25 #photometry bkg outer anulus (in pixels)
Edge_buffer= 250 #to remove stars that are close to edge (in pixels)
...
for k in [11]:#listt:
    ...
    #get list of names of directories corresponding to each stare/observation_session
    os.system(" ls -d {}/*/ > {}temp.txt".format(data_parentpath + stars[k], tempfile_path))
    dirs = [dirpath.split('/')[-2]+'/' for dirpath in open('/home/gagarwal/Downloads/temp.txt', 'r').read().split('\n')[:-1]]

    savefolder = parent_savefolder + stars[k] + '/' #path of the directory where the target's results will be saved
    os.system('mkdir -p '+savefolder)
    radec_file = open(savefolder+'ra_dec.txt', 'w') #this file keeps a record of the ra and dec of all the stars processed in the field
    chosenstaridx_file = open('{}chosen_ref_stars_index.txt'.format(savefolder), 'w') #this file keeps a record of the stars that were chosen to be the reference stars for the target for differential photometry

    #splits each stare/observation_session further if the telescope observing the star changes mid-stare (like in LCO observations). This makes finding the pixel-positions of the stars more robust 
    num_images_pertelescope = individual_telescope_obs()

    #get radec of (most of) the stars in the field.
    radec_stars, num_stars = get_radec_stars()
    
    #to save the relative lightcurve of the target combining all the observations
    alldir_time = []
    alldir_data = []
    ...
    for j in range(0,len(dirs)):
        ...
        center_coord = [int(round(x_length/2)), int(round(y_length/2))]
        ...
        cumulative = 0
        #this for loop records the flux and time data of all the stars
        for i in range(len(num_images_pertelescope[j])):
            ...
            for t in range(cumulative, cumulative+num_images):
                ...
                #use 2-D correlation (probably technically convolution) to find the drifts in stars' pixels positions
                slice_size = int(round(min(x_length, y_length)*.125))
                slice = [center_coord[0] - slice_size, center_coord[1] + slice_size]
                ...
                max_val = np.max(corr)
                max_idx = np.where(corr == max_val) #find where the 2-D correlation peaks
                # print(max_idx)

                shift = np.array([slice_size, slice_size]) - np.array([max_idx[1][0], max_idx[0][0]]) #find the shift in the star field 
                # print(shift)

                currentx, currenty = initialx + shift[0], initialy + shift[1] #update the positions
    ...
    #choose reference stars for the target. The same reference stars will be used for all other stars too
    chosenstars_idx, weights = choose_referencestars()
    sort_arr = np.flip(np.argsort(weights))
    weights = np.array(weights)[sort_arr]
    chosenstars_idx = np.array(chosenstars_idx)[sort_arr] #sort the reference stars in decreasing order of the weights
    for i in range(len(chosenstars_idx)):
            chosenstaridx_file.write('{}\n'.format(chosenstars_idx[i]))
    print('Number of comparison stars chosen for mean lightcurve: ',len(chosenstars_idx))


    rel_lcs=[]
    rel_lc_errs=[]
    #this for loop calculates the relative flux for each stars by dividing each star's normalised flux by the weighted mean lightcurve of the chosen reference stars
    for measured_star in range(0,num_stars):
        star_fluxes=[] 
        star_errs=[]
        
        curr_weights = []
        for i in range(len(chosenstars_idx)):
            if(chosenstars_idx[i] != measured_star):
                star_flux = np.concatenate([dirdata[:,chosenstars_idx[i],2] for dirdata in alldir_data])
                star_fluxes.append(star_flux/np.mean(star_flux)) #append those lightcurves
                curr_weights.append(weights[i])
                star_err = np.concatenate([dirdata[:,chosenstars_idx[i],3] for dirdata in alldir_data])
                star_errs.append(star_err/np.mean(star_flux))

        try: #get weighted average lightcurve
            mean_lc=np.average(star_fluxes,weights=curr_weights, axis=0)
        except ZeroDivisionError:
            mean_lc=np.average(star_fluxes, axis=0)

        weights_sq = [weight**2 for weight in curr_weights]
        mean_lc_err = np.average([np.power(star_err,2) for star_err in star_errs] , weights=weights_sq, axis=0)
        mean_lc_err = mean_lc_err * np.sum(weights_sq) / np.power(np.sum(curr_weights), 2) #errors in the mean LC
        ...
        #for the calculation of the relative flux error, I referred to https://faraday.physics.utoronto.ca/PVB/Harrison/ErrorAnalysis/Propagation.html


    #This loop saves the relative fluxes of all the stars for one stare/observation_session at a time
    for j in range(0,len(dirs)):
        ...
        os.system('mkdir -p '+lightcurve_savepath)    
        ...
        for row in results:  
            ...
            with open(savefile, 'w') as filehandle:
                ...
                filehandle.write('#\n#\n#\n')
                filehandle.write('#filename\ttime\tflux\tflux_err\trel_flux\trel_flux_err\tx\ty\n')
    ...
    #save the relative lightcurve of the target combining all the observations.
    alldir_time = np.concatenate(alldir_time)
    sort_arr = np.argsort(alldir_time)
    rel_lcs = rel_lcs[0][sort_arr]
    rel_lc_errs = rel_lc_errs[0][sort_arr]
    fullflux_dir = '/home/gagarwal/Downloads/full_relative_LCs/'
    os.system('mkdir -p {}'.format(fullflux_dir))
    alldata_combinedfile = open('{}{}.txt'.format(fullflux_dir, stars[k]), 'w')
    alldata_combinedfile.write('ra: {}\tdec: {}\n'.format(radec_stars[results[0][1],0],radec_stars[results[0][1],1]))
    alldata_combinedfile.write('Time(J.D),Relative Flux,Relative Flux Error\n')
    for i in range(alldir_time.shape[0]):
        alldata_combinedfile.write('%f,%f,%f\n' % (alldir_time[i], rel_lcs[i], rel_lc_errs[i]))
