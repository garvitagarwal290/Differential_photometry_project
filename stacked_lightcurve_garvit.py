...
def initialFindFITS(data, detect_thresh):
    ...
    ell_ratio = objects['a']/objects['b']
    

    return positions, ell_ratio
...
data_path = '/home/gagarwal/Downloads/Data/2MASS_J14254686-0359169/all_combined/'
savefolder = '/home/gagarwal/Downloads/relative_LCs/2MASS_J14254686-0359169/'

''' get list of image names to process'''
filenames = sorted(glob(data_path+'*.fits.fz'))#list of mean-stacked frames
...
global Edge_buffer
Edge_buffer= 40 #10 #to remove stars that are close to edge, I think this should be more than 20px

ap_r=5 #aperture radius for photometry
...
positions = initialFindFITS(first_frame[0], detect_thresh)
star_find_results = tuple(positions[0]) #find stars on first frame
ell_ratio = positions[1]

if len(star_find_results)<20: #quit when no stars
    print("Not good image! ", filenames[0])
    sys.exit()

# print(star_find_results[372], ell_ratio[371])    

before_ellipticity_filter = len(star_find_results)
star_find_results = tuple(star_find_results[i] for i in range(before_ellipticity_filter) if ell_ratio[i] < 1.3)
...
for t in trange(1, num_images):#read through all images
    ...
    try:
        x,y=getXY(hdu[1].header, ra_dec)#open fits file, get its wcs and get x y having initial ra dec list
    except ValueError:
        print('no wcs')
        continue
    hdu.close()
...
#%%
    ...
    # score=[]#list with stars and their score
    # median = []
    # stdev=[]
    # star_flux=[x/np.average(data[:,measured_star,2]) for x in data[:,measured_star,2]] #normalized lightcurve of target star
    # for star in range(0, num_stars): #loop through all stars
    ...
    #         score.append([star, abs(np.median(residual)) * np.std(residual)])
    # score.sort(key = lambda x: x[1]) #sort in descending order
    # for star in score[0:50]: #choose 10 stars that have least difference with target lightcurve
    #     star_fluxes.append(data[:,star[0],2]) #append those lightcurves
    #     try: 
    #         # weights.append((stats.sigma_clipped_stats(data[:,star[0],2])[1]/stats.sigma_clipped_stats(data[:,star[0],2])[2])**2)
    #         weights.append((1/stats.sigma_clipped_stats(data[:,star[0],2])[2])**2)
    #     except ZeroDivisionError:
    #         weights.append(1)
    ...
    for star in range(0, len(median)):
        if(median[star] < median_thresh and stdev[star]<stdev_thresh):
            ...
            star_fluxes.append(data[:,star,2]) #append those lightcurves
            try:
                # weights.append((stats.sigma_clipped_stats(data[:,star[0],2])[1]/stats.sigma_clipped_stats(data[:,star[0],2])[2])**2)
                ref_flux=[x/np.average(data[:,star,2]) for x in data[:,star,2]]
                weights.append((1/stats.sigma_clipped_stats(ref_flux)[2])**2)
            except ZeroDivisionError:
                weights.append(1)

    if(measured_star==315):
        print('Number of comparison stars chosen for mean lightcurve: ', len(star_fluxes))
...
if not os.path.exists(lightcurve_savepath):
    os.system('mkdir '+lightcurve_savepath)      #make folder to hold master bias images in
...
for row in results:  # loop through each detected event
    ...
    savefile = lightcurve_savepath+'star' + str(row[1]) + "_" + str(int(star_coords[0])) + "-" + str(int(star_coords[1])) + ".txt"
